...
| **`services/api.ts`** | Axios instance with auth header interceptor; grouped exports: `authAPI` (login, register, checkEmail, logout, forgotPassword, resetPassword), `profileAPI`, `matchingAPI`, `connectionsAPI`, `chatAPI`, `blockAPI`, `feedbackAPI`; `getPhotoUrl()` for relative vs absolute image URLs; default `api` export. |
